...
def check_phone(number):
    number_check = re.compile(r'^\+?1?\d{9,15}$')

    if ((number_check.search(number) == None)):
        raise forms.ValidationError("Niepoprawna wartość pola. Numer powinien być w postaci '999999999'. ")
...
class WithoutDBPhotoForm(forms.Form):
    ...
    birth_date = forms.CharField(label='Data urodzenia', widget=DateOfBirthWidget(order='DMY'))
    # phone = forms.CharField(label='Numer telefonu', max_length=15)
    phone = forms.CharField(max_length=15, validators=[check_phone])
    email = forms.EmailField(label='E-mail', max_length=100)
    examination = forms.ChoiceField(label="Rozpoznanie", choices=VEINS)
    description = forms.CharField(label='Opis', max_length=100, widget=forms.Textarea)
