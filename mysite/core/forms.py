...
def check_pesel(pesel):
    ...
    check = pesel_array[0] * 1 + pesel_array[1] * 3 + pesel_array[2] * 7 \
            + pesel_array[3] * 9 + pesel_array[4] * 1 + pesel_array[5] * 3 + pesel_array[6] * 7 + pesel_array[7] * 9 + pesel_array[8] * 1 + pesel_array[9] * 3
...
def check_phone(number):
    ...
    if ((number_check.search(number) == None)):
        raise forms.ValidationError("Niepoprawna wartość pola. Numer powinien być w postaci '(+48/0048)-xxx-xxx-xxx' lub '(+48/0048) xxx xxx xxx' lub '(+48/0048)xxxxxxxxx'.")
...
class WithoutDBPhotoForm(forms.Form):
    ...
    description = forms.CharField(label='Opis', max_length=100, widget=forms.Textarea, required=False)
